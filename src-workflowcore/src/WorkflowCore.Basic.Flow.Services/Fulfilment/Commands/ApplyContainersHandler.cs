    ...
    public async Task HandleAsync(ApplyContainersCommand cmd, CancellationToken ct = default)
    {
        ...
        for (var attempt = 1; attempt <= maxRetries; attempt++)
        {
            try
            {
                ...
                {
                    logger.LogWarning("[ApplyContainers] Aggregate {Id} not found", cmd.FulfilmentId);
                    return;
                }
                aggregate.UpdateContainers(cmd.Containers);
                await repository.SaveAsync(aggregate, ct);
                logger.LogInformation("[ApplyContainers] {Id} updated with {Count} container(s)", cmd.FulfilmentId, cmd.Containers.Count);
                return;
            }
            catch (ConcurrencyException) when (attempt < maxRetries)
            {
                var delay = TimeSpan.FromMilliseconds(100 * Math.Pow(2, attempt - 1));
                logger.LogWarning("[ApplyContainers] Concurrency conflict (attempt {A}/{Max}), retrying in {Delay}ms", attempt, maxRetries, delay.TotalMilliseconds);
